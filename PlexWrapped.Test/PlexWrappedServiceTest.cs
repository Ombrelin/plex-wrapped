...
namespace PlexWrapped.Test
{
    public class PlexWrappedServiceTest
    {
        ...
        public async Task GetMostPlayedArtists_ThreeArtists_ReturnsThreeMostPlayedArtistsOrderedByPlayCount()
        {
            ...
            var plexWrappedService =
                new WrappedService(plexApiMock.Object, tautulliApiMock.Object);
            ...
            Assert.Equal("http://192.192.192.192:8080/library/metadata/1/thumb?X-Plex-Token=token", result[0].ThumbnailUrl);

            Assert.Equal("Test Artist 2", result[1].Name);
            Assert.Equal("http://192.192.192.192:8080/library/metadata/2/thumb?X-Plex-Token=token", result[1].ThumbnailUrl);

            Assert.Equal("Test Artist 3", result[2].Name);
            Assert.Equal("http://192.192.192.192:8080/library/metadata/3/thumb?X-Plex-Token=token", result[2].ThumbnailUrl);
        }
    }
}